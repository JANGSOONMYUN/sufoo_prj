...
def _valid_chain_in_out(chain_key, chain_val, chains, keep_data_set):
    if chain_val is None:
        return True
    output_keys = chains[chain_key]['output_dict']
    # save keep_input_output_data
    if 'keep_input_output_data' in chains[chain_key]:
        for kp_data in chains[chain_key]['keep_input_output_data']:
    ...
    for k, v in chain_val.items():
        ...
        if output_keys is None or len(output_keys) == 0:
            ...
            for in_k in input_keys:
                if in_k not in output_keys.keys() and in_k not in keep_data_set:
